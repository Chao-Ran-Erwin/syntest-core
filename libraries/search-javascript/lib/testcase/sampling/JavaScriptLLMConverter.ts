...
export class JavaScriptLLMConverter extends JavaScriptTestCaseSampler {
  ...
  override sampleFunctionCall(depth: number, data?: CallExpressionData): FunctionCall {
    if (!data) {
      JavaScriptLLMConverter.LOGGER.warn("CallExpression data is required for FunctionCall.");
      return undefined;
    }

    // Extract the function name from the callee
    if (data.callee.type !== "Identifier") {
      JavaScriptLLMConverter.LOGGER.warn(`Expected Identifier for FunctionCall, but got ${data.callee.type}.`);
      return undefined;
    }
    ...
    const functionTarget = <FunctionTarget>(targets.find((t) => (t as FunctionTarget).name === functionName));
    if (!functionTarget) {
    ...
    for (const [index, argument] of data.args.entries()) {
      ...
      const mappedArgument = this._mapArgument(depth + 1, argument, parameterId, parameterId, name);
      if (!mappedArgument) {
    ...
    return new FunctionCall(functionTarget.id, functionTarget.typeId, functionTarget.name, prng.uniqueId(), arguments_, export_);
  }

  override sampleConstructorCall(depth: number, classId?: string, data?: ConstructorCallData, name?: string): ConstructorCall {
    if (name && this.statementMap.has(name)) {
      return this.statementMap.get(name) as ConstructorCall;
    }
    // Case 1: Construct from IR data
    if (data) {
      const class_ = <ClassTarget>((<JavaScriptSubject>this._subject)
        .getActionableTargetsByType(TargetType.CLASS)
        .find((t) => (t as ClassTarget).name === data.callee));
      if (!class_) {
        JavaScriptLLMConverter.LOGGER.warn(`Class target not found for: ${JSON.stringify(data.callee)}`);
        return undefined;
      }

      const constructor_ = (<JavaScriptSubject>this._subject)
        .getActionableTargetsByType(TargetType.METHOD)
        .find((method) => (<MethodTarget>method).classId === class_.id && (<MethodTarget>method).methodType === "constructor");

      if (!constructor_) {
        JavaScriptLLMConverter.LOGGER.warn(`In sampleConstructorCall Constructor not found for class: ${class_.id}`);
        return undefined;
      }
      ...
      for (const [index, argument] of data.args.entries()) {
        ...
        // Map the argument to a Statement
        const value = this._mapArgument(depth + 1, argument, parameterId, parameterId, name);
      ...
      const constructorCall = new ConstructorCall(constructor.id, constructor.typeId, class_.id, name, prng.uniqueId(), arguments_, export_);

      this.statementMap.set(name, constructorCall);
      return constructorCall;
    }

    JavaScriptLLMConverter.LOGGER.warn("Either data or classId must be provided for sampleConstructorCall.");
    return undefined;
  }

  sampleClassAction(depth: number): MethodCall | Getter | Setter {
    JavaScriptLLMConverter.LOGGER.warn("sampleClassAction not implemented: " + depth);
    return undefined;
  }

  override sampleMethodCall(depth: number, data?: CallExpressionData): MethodCall {

    if (!data) {
      JavaScriptLLMConverter.LOGGER.warn("CallExpression data is required for MethodCall.");
      return undefined;
    }
    ...
    const methods = (<JavaScriptSubject>this._subject)
      ...
      .filter((target) => isExported(targets.find((objectTarget) => objectTarget.id === (<MethodTarget>target).classId)));

    // Retrieve the method target
    const methodTarget = <MethodTarget>(methods.find((t) => (t as MethodTarget).name === methodName));
    ...
    if (!objectName) {
      JavaScriptLLMConverter.LOGGER.warn("Unable to extract base object name from callee");
      return undefined;
    }
    ...
    if (!constructorCall) {
      ...
      constructorCall = this.sampleConstructorCall(depth + 1, class_.id, undefined, objectName);
      if (constructorCall) {
        this.statementMap.set(objectName, constructorCall);
      } else {
        JavaScriptLLMConverter.LOGGER.warn(`Could not create constructor call for object: ${objectName}`);
        return undefined;
      }
    }
  ...
  sampleGetter(depth: number, name?: string, data?: MemberExpressionData): Getter {
    const propertyName = data.property as string;
    const property = [...(<JavaScriptSubject>this._subject)
      .getActionableTargetsByType(TargetType.PROPERTY), ...(<JavaScriptSubject>this._subject)
      .getActionableTargetsByType(TargetType.METHOD)
    ...
    if (typeof data.object === "object" && "data" in data.object && (data.object.data as { name?: string }).name) {
      ...
      JavaScriptLLMConverter.LOGGER.error("Unsupported object type in MemberExpressionData");
      return undefined;
    }

    const constructor_ = this.statementMap.get(constructorName) as ConstructorCall;
    if (!constructor_) {
      JavaScriptLLMConverter.LOGGER.warn(`In sampleGetter Constructor call not found for class: ${constructorName}`);
      return undefined;
    }
    if (constructor_.classIdentifier !== (property as PropertyTarget).classId) {
      JavaScriptLLMConverter.LOGGER.error(`Wrong constructor class in sampleGetter ${constructor_.classIdentifier}`);
      return undefined;
    }
    return new Getter(property.id, property.id, propertyName, prng.uniqueId(), constructor_);
  }

  sampleSetter(depth: number, left?: Getter, right?: Statement): Setter {
    // NOTE: if left or right is undefined, log and return undefined.
    if (!left || !right) {
      JavaScriptLLMConverter.LOGGER.warn("Setter missing left getter or right statement");
      return undefined;
    }
    return new Setter(left.variableIdentifier, left.typeIdentifier, left.name, prng.uniqueId(), right, left.constructor_);
  }

  sampleConstantObject(depth: number, objectId?: string): ConstantObject {
    JavaScriptLLMConverter.LOGGER.warn("sampleConstantObject not implemented: " + depth + objectId);
    return undefined;
  }

  sampleObjectFunctionCall(depth: number): ObjectFunctionCall {
    JavaScriptLLMConverter.LOGGER.warn("sampleObjectFunctionCall not implemented: " + depth);
    return undefined;
  }

  sampleArrayArgument(depth: number, arrayId: string): Statement {
    JavaScriptLLMConverter.LOGGER.warn("sampleArrayArgument not implemented: " + depth + arrayId);
    return undefined;
  }

  sampleObjectArgument(depth: number, objectId: string, property?: string): Statement {
    JavaScriptLLMConverter.LOGGER.warn("sampleObjectArgument not implemented: " + depth + objectId + property);
    return undefined;
  }
  ...
  sampleObject(depth: number, id: string, typeId: string, name: string, data?: ObjectExpressionData): ObjectStatement {
    ...
    for (const [key, value] of Object.entries(data.properties)) {
      ...
      if (mapped) {
        ...
        JavaScriptLLMConverter.LOGGER.warn(`Property ${key} could not be mapped in Object.`);
      }
    }
  ...
  sampleArray(depth: number, id: string, typeId: string, name: string, data?: IRStatement[]): ArrayStatement {
    const elements: Statement[] = (data || [])
      .map((statement) => this._mapArgument(depth + 1, statement, "id", "typeid", "arrayElement"))
  ...
  sampleString(id: string, typeId: string, name: string, value?: string): StringStatement {
    return new StringStatement(id, typeId, name, prng.uniqueId(), value);
  }

  sampleBool(id: string, typeId: string, name: string, value?: boolean): BoolStatement {
  ...
  sampleNumber(id: string, typeId: string, name: string, value?: number): NumericStatement {
    return new NumericStatement(id, typeId, name, prng.uniqueId(), value);
  }

  sampleInteger(id: string, typeId: string, name: string, value?: number): IntegerStatement {
  ...
  private _processIRStatements(depth: number, irStatements: IRStatement[]): ActionStatement[] {
      ...
      try {
        switch (irStatement.type) {
          case "ConstructorCall": {
            const stmt = this.sampleConstructorCall(depth, undefined, irStatement.data as ConstructorCallData);
          ...
          case "CallExpression": {
            ...
            if (data.callee.type === "Identifier") {
              ...
              const calleeCallExpression = calleeData.object.data as CallExpressionData;

              // Expect case
              if (calleeData.object.type === "CallExpression" && calleeCallExpression.callee.type === "Identifier" && (calleeCallExpression.callee.data as {
                name: string;
              }).name === "expect") {
                // Extract the method call inside `expect`
                const innerCall = calleeCallExpression.args[0];
                const innerCallExpression = innerCall.data;
                if (innerCall.type === "CallExpression" && (innerCallExpression as CallExpressionData).callee.type === "MemberExpression") {
                  const stmt = this.sampleMethodCall(depth + 1, innerCallExpression as CallExpressionData);
                  if (stmt) processedStatements.push(stmt);
                } else if (innerCall.type === "MemberExpression") {
                  const stmt = this.sampleGetter(depth + 1, (innerCallExpression as MemberExpressionData).property as string, innerCallExpression as MemberExpressionData);
                  if (stmt) processedStatements.push(stmt);
                  ...
                  if (this.statementMap.has(identifierName)) {
                    ...
                    JavaScriptLLMConverter.LOGGER.warn(`Unhandled Identifier in expect: ${identifierName}`);
                  }
                } else {
                  JavaScriptLLMConverter.LOGGER.warn(`Unhandled expect inner call: ${JSON.stringify(innerCall)}`);
                }
              } else {
              ...
              JavaScriptLLMConverter.LOGGER.warn("Invalid CallExpression callee type");
            }
            break;
          }
          case "VariableDeclaration": {
            ...
            if (!data.init) {
              JavaScriptLLMConverter.LOGGER.warn(`VariableDeclaration '${data.name}' has no initializer.`);
              break;
            }
            // Use _mapArgument with the variable name
            const initializer = this._mapArgument(depth, data.init, undefined, undefined, data.name);
          ...
          case "AssignmentExpression": {
            ...
            if (data.left.type === "Identifier") {
              ...
              const rightStatement = this._mapArgument(depth, data.right, undefined, undefined, variableName);
              ...
              JavaScriptLLMConverter.LOGGER.warn(`Unsupported left type in AssignmentExpression: ${data.left.type}`);
            }
            break;
          }
          default: {
            JavaScriptLLMConverter.LOGGER.warn(`Unhandled IR statement type: ${irStatement.type}`);
            JavaScriptLLMConverter.LOGGER.warn(JSON.stringify(irStatement.data));
          }
        }
      } catch (error) {
        JavaScriptLLMConverter.LOGGER.error(`Error processing IR statement: ${error}`);
      }
    }
   ...
   * @param name
   */
  private _mapArgument(depth: number, argument: IRStatement, id = "id", typeId = "typeId", name = "anon"): Statement | undefined {
    try {
      switch (argument.type) {
        ...
        case "MemberExpression": {
          ...
          // Resolve the object
          const objectName = data.object.type === "Identifier" ? (data.object.data as { name: string }).name : (() => {
            JavaScriptLLMConverter.LOGGER.warn(`Unsupported object type in MemberExpression: ${data.object.type}`);
            return "unknown";
          })();
          // Attempt to resolve the property dynamically
          const resolvedValue = this._resolveConstantValue(objectName, data.property as string);
          if (resolvedValue !== undefined) {
            ...
            if (typeof resolvedValue === "number") {
              ...
              JavaScriptLLMConverter.LOGGER.warn(`Unhandled constant value type: ${typeof resolvedValue}`);
              return undefined;
            }
          }
        ...
        case "ObjectExpression": {
          return this.sampleObject(depth + 1, id, typeId, name, argument.data as ObjectExpressionData);
        }
        case "ArrayExpression": {
          return this.sampleArray(depth + 1, id, typeId, name, argument.data as IRStatement[]);
        }
        case "Identifier": {
          ...
          JavaScriptLLMConverter.LOGGER.warn(`Unhandled Identifier: ${identifierName}`);
          return undefined;
        }
        default: {
          JavaScriptLLMConverter.LOGGER.warn(`Unhandled argument type: ${argument.type} ${JSON.stringify(argument.data)}`);
          return undefined;
        }
      }
    } catch (error) {
    ...
    try {
      switch (callee.type) {
        case "MemberExpression": {
          ...
          JavaScriptLLMConverter.LOGGER.warn("Computed property names are not supported.");
          return "unknown";
        }
        case "Identifier": {
        ...
        default: {
          JavaScriptLLMConverter.LOGGER.warn(`Unsupported callee type: ${callee.type}`);
          return "unknown";
        }
      }
    } catch (error) {
      JavaScriptLLMConverter.LOGGER.error(`Error in _extractMethodName: ${error}`);
      return "unknown";
    }
  }
  ...
  private _getClass(id?: string): ClassTarget | undefined {
    if (id) {
      const result = <ClassTarget>((<JavaScriptSubject>this._subject)
        .getActionableTargetsByType(TargetType.CLASS)
      ...
      if (!result) {
        ...
        JavaScriptLLMConverter.LOGGER.error("Class with id: " + id + " is not exported");
        return undefined;
      }
      return result;
    }
    JavaScriptLLMConverter.LOGGER.error("No id provided to _getClass; cannot pick one randomly");
    return undefined;
  }
  ...
  private _getExport(classId: string | undefined): Export {
    ...
    if (!exp) {
      JavaScriptLLMConverter.LOGGER.warn(`Export not found for classId: ${classId}`);
    }
    return exp;
  }

  private _resolveConstantValue(objectName: string, property: string): number | string | boolean | undefined {
    const constantMappings: Record<string, Record<string, number | string | boolean | undefined>> = {
      Number: {
